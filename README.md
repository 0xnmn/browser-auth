...
The same target works for every browser operation:

```ts
const target = { cdpUrl, url: "https://service.example" };
auth.login({ ...target, save: "never" });
auth.switchAccount({ ...target, accountId });
auth.logout(target);
auth.logout({ ...target, accountId, forgetCredentials: true });
...
Use a trusted `.mjs` module exporting the same `AuthOptions` you would pass to `createAuth` (configured `model`, optional shared persistent store/tracer). Do not put credentials in command-line arguments or commit provider keys.
...
From this checkout: `node packages/browser-auth/dist/cli/main.js --help`. Passwords/codes are masked. Ctrl+C cancels. Exit codes: 0 confirmed success, 1 runtime/configuration/input failure or unknown outcome or save/delete failure, 2 invalid command syntax, 130 cancellation before browser mutation. By default, saved credentials disappear when the CLI process exits; cross-invocation reuse requires a host-provided persistent store. Account commands do not require CDP; listing also supports `--credential-origin` and returns metadata only.

All operation options are accepted through `--input /private/options.json`: `cdpUrl`, `url`, `cdpHeaders`, `targetId`, partial `credentials`, `accountId`, and the operation's save/label/forget options. Only fields valid for that operation are accepted. Keep secret files out of Git and readable only by the owner; this is input transport, not a credential file store. Explicit flags override file fields, and `BROWSER_AUTH_CDP_URL` is used only as a fallback. `--target-id` selects a specific tab. Model configuration, stores, limits, and tracing work through the same `AuthOptions` configuration as the SDK.
...
| ------------------------------------ | ---------------------------------------------------------- |
| `login`, `switchAccount`, `logout`   | `login`, `switch`, `logout`                                |
