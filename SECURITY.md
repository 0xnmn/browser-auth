...
- Private values are held by host/controller code and are never included in model observations or proposal tool arguments. Known values are redacted from observed text and metadata. This is best-effort known-value filtering, **not a general DLP boundary**. Authenticated page text, identifiers, and pre-existing browser data can still reach the configured model/provider.
- Models make one named, schema-validated AI SDK tool call. The tools have no execute callbacks and expose no arbitrary JavaScript, cookies, storage, or network access. The controller alone binds private fields, fills captured controls, submits forms, and dispatches native choices. Routine authentication clicks execute automatically; provider, account, and method classification remains model-inferred, not a security boundary.
- Observations contain a bounded DOM hierarchy and SDK-owned captured references. Editable controls are masked in screenshots before private values are known; after the redactor holds any private value, screenshots are withheld. Text redaction and screenshot policy reduce accidental reflection but cannot promise perfect secrecy.
...
- No raw exception serialization in flow output. Opt-in OTLP traces use fixed names and metadata. Model-adapter telemetry is disabled. Never log raw UI responses, store calls, CDP endpoints, or provider payloads.
- `flow.transcript()` and CLI `--transcript <file>` are separate diagnostic content streams, not OTLP telemetry. They include observed website/account content, validated proposals, sanitized execution outcomes/errors, and safe observation screenshots. Accepted field responses include identifiers only, not values. Known reflected credentials are filtered structurally, but earlier events/images are not retroactively sanitized. Protect transcript access, storage and retention; do not send real-account transcripts to public logs or issues. CLI files use exclusive creation and mode `0600`; this is not encryption or a hostile-site confidentiality guarantee.
...
A website receiving a credential can read it, forward it, change form destinations, encode it, or reflect it in forms or page content. A hostile page can deliberately reveal a value after it receives it; known-string redaction is not semantic taint tracking. The library cannot guarantee confidentiality of reflected values or pre-existing cookies, storage, account data, page text, or screenshots captured before private values are known. It also cannot isolate credentials from other CDP clients, extensions, browser instrumentation, same-process code, heap dumps, or provider instrumentation installed by the caller. Do not use an untrusted page just because its origin was displayed. Login, logout, and account-change outcomes are inferred by the model from the page, not independently verified identity or session proof. A returned `accountId` identifies a saved credential record, not the browser's current account.
